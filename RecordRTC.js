...
function StereoAudioRecorder(mediaStream, config) {
  ...
  function onAudioProcessDataAvailable(e) {
    ...
    for (var j = 0; j < left.length; j++) {//floatTo16BitPCM 
      var s = Math.max(-1, Math.min(1, left[j]));
      s = s < 0 ? s * 0x7000 : s * 0x6FFF;
      sum += Math.abs(s);
    };
  ...
  function looper() {
    ...
    if (intervalsBasedBuffers.left.length) {
      ...
      mergeLeftRightBuffers({
        ...
        config.ondataavailable(blob, leftchannel, PowerLevel(sum, leftchannel[leftchannel.length - 1].length));
